"""
Video Filter Application with Real-Time Playback and Feature Detection

Desktop application for video processing with 22+ real-time filters
and computer vision feature detection tools.

Core functionality:
- Video file and webcam support
- Real-time playback with frame-by-frame controls
- 22 combinable filters (edge detection, blur, color effects, artistic styles)
- Frame gallery for saving and analyzing frames
- Feature detection: HOG, ORB, SIFT, SURF, Canny
- Augmentation robustness testing for CV algorithms

Built with OpenCV, Tkinter, and NumPy.
"""
...
class VideoFilterApp:
    ...
    def open_augmentation_evaluation(self):
        ...
        if detector_choice == "none":
            messagebox.showinfo("Info", "Please select a feature detector first (ORB, SIFT, SURF, FERIF).")
            return
        ...
        if detector_choice in ["hog", "canny"]:
            messagebox.showerror(
                ...
                f"Please use: ORB, SIFT, SURF, or FERIF\n\n"
        ...
        def run_evaluation():
            try:
                ...
                def detector_func(image):
                    ...
                    elif detector_choice == "ferif":
                        from ferif import ferif_detect
                        kp, desc = ferif_detect(image)
                        return kp, desc
                    else:
                        # Fallback to ORB
                        orb = cv2.ORB_create(nfeatures=MAX_KEYPOINTS_ORB)
